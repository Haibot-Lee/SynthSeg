...
synthseg_home = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
def process_imgs(args):
    ...
    if args['robust']:
        args['fast'] = True
        assert not args[
            'v1'], 'The flag --v1 cannot be used with --robust since SynthSeg-robust only came out with 2.0.'
        version = 'SynthSeg-robust 2.0'
    else:
        version = 'SynthSeg 1.0' if args['v1'] else 'SynthSeg 2.0'
        if args['fast']:
            version += ' (fast)'
    logger.info("SynthSeg version: %s" % version)

    # enforce CPU processing if necessary
    if args['cpu']:
        logger.info('using CPU, hiding all CUDA_VISIBLE_DEVICES')
        os.environ['CUDA_VISIBLE_DEVICES'] = '-1'

    # limit the number of threads to be used if running on CPU
    logger.info('using %s threads' % args['threads'])
