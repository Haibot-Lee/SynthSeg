...
labels = np.load('../data_training/labels/generation_labels.npy')
mapping_df = pd.read_csv('../data_training/labels/labels_table.csv')
mapping_df.set_index("labels", inplace=True)
label_names = [mapping_df.loc[i, "structs"] for i in labels]
seg_names = np.array(label_names)
np.save('../data_training/labels/segmentation_names.npy', seg_names)
